...
ConvState.prototype.printAnswers = function (answers, multiple) {
    ...
    $(this.wrapper).find('#messages').css('paddingBottom', '5px');
};
ConvState.prototype.answerWith = function (answerText, answerObject) {
    // console.log('in answer: ', answerText, answerObject);
...
(function ($) {
    $.fn.convform = function (placeholder) {
        ...
        var inputForm = $('<form id="convForm"><div class="options dragscroll"></div><div class="text-input"><textarea id="userInput" rows="1" placeholder="' + placeholder + '"></textarea><button type="submit" class="icon2-arrow submit">⯈</button></div><span class="clear"></span></form>');
        ...
        $(inputForm).find('button.submit').click(function (e) {
            e.preventDefault();
            var answer = $(inputForm).find('#userInput').val();
            $(inputForm).find('#userInput').val("");
            if (state.current.input.type == 'select') {
...
$(function () {
    ...
    var convForm = $('.conv-form-wrapper').convform("Пишувајте тука...");
});